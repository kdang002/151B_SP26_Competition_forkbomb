...
def extract_last_boxed(text: str) -> str | None:
    """Extracts the content of the last \\boxed{...} in a string."""
...
def get_majority_voted_response(output):
    ...
    for o in output.outputs:
        ...
        if boxed_answer is not None:
            ...
            normalized_answer = judger.norm_math_str(boxed_answer)
            candidates.append((normalized_answer, o.text.strip()))
    ...
    answer_counts = Counter(norm_ans for norm_ans, full_text in candidates)
    most_common_answer = answer_counts.most_common(1)[0][0]
    
    # Return the full text of the first response that gave the majority answer
    for norm_ans, full_txt in candidates:
...
print("Processing responses with optimized majority voting...")
with ProcessPoolExecutor() as executor:
    responses = list(executor.map(get_majority_voted_response, outputs))
